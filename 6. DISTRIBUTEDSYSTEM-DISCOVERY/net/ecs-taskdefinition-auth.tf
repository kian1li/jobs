resource "aws_ecs_task_definition" "auth" {
  family                   = "${var.default_tags.project}-auth"
  ...
  container_definitions = jsonencode([
    {
      name             = "auth"
      image            = "${var.user_id}.dkr.ecr.ap-northeast-2.amazonaws.com/auth-app"
      cpu              = 0
      essential        = true
      task_role_arn    = aws_iam_role.ecs_task_execution_role.arn
      logConfiguration = local.auth_logs_configuration
      portMappings = [
        {
        ...
        {
          containerPort = var.auth_grpc_port
          hostPort      = var.auth_grpc_port
          protocol      = "tcp"
        }
      ],
      environment = [
