resource "aws_ecs_task_definition" "file" {
  family                   = "${var.default_tags.project}-file"
  ...
  container_definitions = jsonencode([
    {
      name          = "file"
      image         = "${var.user_id}.dkr.ecr.ap-northeast-2.amazonaws.com/file-app"
      cpu           = 0
      essential     = true
      task_role_arn = aws_iam_role.ecs_task_execution_role.arn
      logConfiguration = local.file_logs_configuration
      portMappings = [
        {
        ...
        {
          containerPort = var.file_grpc_port
          hostPort      = var.file_grpc_port
          protocol      = "tcp"
        }
      ],
      environment = [
